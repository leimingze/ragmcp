...
- [x] ✅ **1.8.2 实现限流中间层** 🟡 P1
  ...
  - **TDD Cycle**:
    ...
    **Test 2: 时间窗口后重置**
    - **RED**:
      - 编写 `test_rate_limit_resets_after_window()`
      - 断言: 等待 time_window 后可以继续调用
    - **Verify RED**: 运行测试确认失败
    - **GREEN**: 实现滑动窗口逻辑
    - **Verify GREEN**: 运行测试确认通过
...
- [x] ✅ **1.8.3 实现日志中间层** 🟡 P1
  ...
  - **TDD Cycle**:
    ...
    **Test 3: 异常日志记录**
    - **RED**:
      - 编写 `test_log_call_records_exception()`
      - 断言: 异常被正确记录并重新抛出
    - **Verify RED**: 运行测试确认失败
    - **GREEN**: 添加异常处理逻辑
    - **Verify GREEN**: 运行测试确认通过
...
- [x] ✅ **2.1.1 实现 AzureOpenAILLM** 🔴 P0
  ...
  - **验收标准**:
    - [x] 每个测试都先失败，观察失败原因正确
    - [x] API 调用格式符合 Azure 规范
    - [x] 支持流式输出（如需要）
    - [x] 测试输出无警告/错误
  - **devspec 参考**: 第 21 行、第 266 行

- [x] ✅ **2.1.2 实现 OpenAILLM** 🟡 P1
